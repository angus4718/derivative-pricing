...
def derivative(func, x0, dx=1.0, n=1, args=(), order=None):
    """Wrapper using numdifftools.Derivative to match scipy.misc.derivative signature.

    - func: callable f(x, *args)
    - x0: point at which to evaluate
    - dx: step size (mapped to `step` in numdifftools)
    - n: derivative order
    - args: extra args passed to func
    """
    if args:
        f = lambda x: func(x, *args)
    else:
        f = func
    der = nd.Derivative(f, n=n, step=dx)
    return der(x0)
...
class Vol_Calculation:
                ...
                if x.size > 0 and y.size > 0:
                    ...
                    def residuals(params):
                        ...
                        if delta**2 - 2 * gamma * sigma_atm**2 >= 0:
                            return 1e20
                        return np.sum(
                            (y - self.implied_vol_curve_func(x, *params)) ** 2
                        )
    ...
    def load_data(self, excel_file, indices_names):
        self.implied_vol_dfs, self.exercise_dates = self.excel_to_implied_vol_dfs(
            excel_file, indices_names
        )
    ...
    def implied_to_local_vol(self, x_0, index_no, params_dfs, exercise_date):
        ...
        def w(x):
            return self.implied_vol_curve_func(
                x, sigma_atm, delta, gamma, kappa
            ) * self.daysinbetween3(exercise_date)

        y = (
            x_0
            - (
                sum(self.r[index_no][: self.daysinbetween3(exercise_date)])
                - sum(self.d[index_no][: self.daysinbetween3(exercise_date)])
            )
            * self.dt
        )
        ...
        local_var = self.implied_vol_curve_func(x_0, sigma_atm, delta, gamma, kappa) / (
            1
            - y / w(x_0) * dw_dy
            + 0.25 * (-0.25 - 1 / w(x_0) + (y / w(x_0)) ** 2) * (dw_dy) ** 2
            + 0.5 * d2w_dy2
        )
    ...
    def interpolated_local_vol(self, x_0, index_no, day, params_dfs):
        ...
        for exercise_date in self.exercise_dates[index_no]:
            spot_moneyness = (
                x_0
                + (sum(self.r[index_no][:day]) - sum(self.d[index_no][:day])) * self.dt
            )
            y = (
                spot_moneyness
                - (
            ...
            local_vol_value = self.implied_to_local_vol(
                y, index_no, params_dfs, exercise_date
            )
